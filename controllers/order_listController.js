...
const getAdminOrderListDetail = async (req, res) => {
  try {
    ...
    if (!orderIdInfo) {
      const cancelIdInfo = await Cancel.findOne({ 'payments.orderId': orderId });

      if (!cancelIdInfo) return res.status(404).json('주문번호가 없습니다.');
      // true이면, 아래 진행
      res.status(200).json(cancelIdInfo);
    } else {
      res.status(200).json(orderIdInfo);
    }
  } catch (err) {
    console.error(err);
    res.status(500).json('알 수 없는 오류');
